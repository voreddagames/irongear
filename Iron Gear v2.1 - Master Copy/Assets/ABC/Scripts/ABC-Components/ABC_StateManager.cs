...
public class ABC_StateManager : MonoBehaviour {
    ...
    public class HitAnimation {
        ...
        private IEnumerator StartAndStopAnimationRunner(ABC_IEntity Entity) {
            ...
            if (this.hitAnimationRunnerClips.Count == 0 || AnimationRunner == null)
                yield break;


            //Get random clip from the list
            AnimationClip animationClip = this.hitAnimationRunnerClips[Random.Range(0, this.hitAnimationRunnerClips.Count()-1)].AnimationClip;
        ...
        /// </summary>
        /// <param name="Entity">Entity hit animation is running on</param>
        public void ActivateAnimation(ABC_IEntity Entity) {
            ...
            ABC_Utilities.mbSurrogate.StartCoroutine(this.StartAndStopAnimationRunner(Entity));
        }
    ...
    public IEnumerator AddEffects(string AbilityName, List<Effect> effects, ABC_IEntity AfflictingOriginator, AbilityType AbilityType, bool IsEffectLink, GameObject Projectile = null, GameObject ObjectHit = null, Vector3 AbilityHitPoint = default(Vector3), bool ActivateAnimationFromHit = true, float ActivateAnimationFromHitDelay = 0f, AnimationClip ActivateAnimationFromHitClip = null, string SpecificAnimationToActivate = "", bool OverrideWeaponBlocking = false, bool ReduceWeaponBlockDurability = true, bool OverrideWeaponParrying = false, float PushAmount = 0f, float LiftAmount = 0f, float PushDelay = 0f) {
        ...
        if (PushAmount > 0 || LiftAmount > 0) {
            meEntity.Push(PushAmount, AbilityHitPoint, 0.5f, LiftAmount, PushDelay);
        }
    ...
    private void ActivateHitAnimation() {
        ...
        selectedHitAnimation.ActivateAnimation(meEntity);
