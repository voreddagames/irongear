...
public class ABC_Controller : MonoBehaviour {
    ...
    public int GetAbilityNextAvaliableComboID(int AbilityID, bool AIActivated = false) {
        ...
        List<ABC_Ability> potentialComboAbilities = this.CurrentAbilities.Where(a => a.abilityEnabled == true && a.abilityCombo == true && a.keyInputType == ability.keyInputType).ToList();
    ...
    public bool IsAbilityLastInComboChain(int AbilityID) {
        ...
        //get a list of all our potential combo abilities which are
        // Enabled, Set as Ability combos, With same input type as ability provided
        List<ABC_Ability> potentialComboAbilities = this.CurrentAbilities.Where(a => a.abilityEnabled == true && a.abilityCombo == true 
    ...
    public void RemoveFromActiveMeleeAbilities(ABC_Ability Ability, int AbilityObjDisabledCountToRemove = 0) {
        ...
        if (AbilityObjDisabledCountToRemove > 0 && this.activeMeleeAbilities[Ability].Where(obj => obj.activeInHierarchy == false).Count() < AbilityObjDisabledCountToRemove)
            return;
    ...
    private bool ButtonPressed(ControllerButtonPressState State, ControllerButtonPressType PressType = ControllerButtonPressType.Press) {
        ...
        if (pressType == ControllerButtonPressType.Press) {
            ...
            if (scrollWheelUpIncluded == true && ABC_InputManager.GetAxis("Mouse ScrollWheel") > 0)
                return true;

            //If scrollwheel down included and being 'pressed' then return true 
            if (scrollWheelDownIncluded == true && ABC_InputManager.GetAxis("Mouse ScrollWheel") < 0)
                return true;
    ...
    private void TargetDeselectHandler() {
        ...
        if (targetObject.activeInHierarchy == false || this.InRange(targetObject, targetSelectRange) == false || entity.HasABCStateManager() == true && entity.healthValue <= 0 || this.enableAI == false && this.ButtonPressed(ControllerButtonPressState.ConfirmSoftTarget))
            this.RemoveTarget();
    ...
    private void AINavigationWanderHandler() {
        ...
        if (this.navAIEnabled == false || this.blockNavAI == true || this.meNavAgent.enabled == false || this.meTransform.gameObject.activeInHierarchy == false || this.IsActivatingAbility() == true)
            return;
    ...
    private void StartAnimationRunner(ControllerAnimationState State, ABC_AnimationsRunner AnimationRunner){
        ...
        AnimationRunner.StartAnimation(animationClip, animationClipDelay, animationClipSpeed, animationClipMask, true, blockRunnerOverrides, false, currentAnimatorClipOverride);
