...
public class ABC_WeaponPickUp : MonoBehaviour {
    ...
    private IEnumerator PickUpWeapon() {
        ...
        foreach (ABC_IEntity Entity in this.collidingEntities.ToList()) {
            ...
            if (this.pickUpMode != EnableOrImport.ImportAsDisabled) {
            ...
            switch (this.pickUpMode) {
                case EnableOrImport.Enable:
                    ...
                    break;
                case EnableOrImport.Import:
                    //else we want to import the weapon from the global portal
                    this.GlobalElementToImport.ElementWeapon.globalWeaponOverrideEnableStatus = true;
                    yield return StartCoroutine(this.ImportWeapon(Entity, this.equipWeaponOnPickup));
                    break;
                case EnableOrImport.ImportAsDisabled:
                    //else we want to import the weapon from the global portal but as disabled (good for GC inventory integration)
                    this.GlobalElementToImport.ElementWeapon.globalWeaponOverrideEnableStatus = false;
                    this.GlobalElementToImport.ElementWeapon.weaponEnabled = false; 
                    yield return StartCoroutine(this.ImportWeapon(Entity, false));
                    break;
            }
