...
public class ABC_MovementController : MonoBehaviour {
    ...
    /// <summary>
    ///  My additional code: Extra Gravity Force
    /// </summary>
    ...
    /// </summary>
    public KeyCode tempLockOffButton = KeyCode.Mouse1;
    ...
    private float VerticalVelocity {
        get {
            ...
            if (this.IsGrounded()) {
                ...
                {
                    _verticalVelocity += Physics.gravity.y * gravityForce * Time.deltaTime; // else If the entity is not grounded and allow gravity is enabled then apply the gravity value to the vertical velocity
                }
            }
    ...
    bool IsInputAvailable(string InputName)
    {
        try
        {
            ABC_InputManager.GetAxis(InputName);
            return true;
        }
        catch 
        {
    ...
    private void DetermineMoveDirection() {
        ...
        //Get input from user
        this.inputX = ABC_InputManager.GetAxis("Horizontal") * this.speedForce;
        //this.inputZ = ABC_InputManager.GetAxis("Vertical") * this.speedForce;
    ...
    private void RotateAndMoveEntity() {
        ...
        else if (this.lockOnTarget == null && Time.time - timeMovementLastDisabled > 0.3f)
    ...
    private IEnumerator TempLockOffHandler(){
        ...
        if (ABC_InputManager.GetKey(this.tempLockOffButton) && this.lockOnTarget != null){
