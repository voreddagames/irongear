...
public static class ABC_Utilities {
    ...
    public static GameObject GetObjectBelowEntity(ABC_IEntity Entity) {
        ...
        foreach (ABC_IEntity nearbyEntity in GetAllABCEntitiesInRange(new Vector3(Entity.transform.position.x, Entity.transform.position.y - 1.5f, Entity.transform.position.z), 2f, true)){
            ...
            //If we are above the entity or as an extra check if the bounds are intersecting then return the object
            if (Vector3.Dot(Entity.transform.position - nearbyEntity.transform.position, nearbyEntity.transform.up) > 0 && Vector3.Dot(Entity.transform.position - nearbyEntity.transform.position, nearbyEntity.transform.up) < 2.5) {
