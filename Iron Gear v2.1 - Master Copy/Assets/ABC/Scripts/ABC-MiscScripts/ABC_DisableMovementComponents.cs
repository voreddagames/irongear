...
public class ABC_DisableMovementComponents : MonoBehaviour {
    ...
    private void HaltNavAgent() {
        ...
        //If nav agent is off ground or falling then turn it off 
        if (ABC_Utilities.EntityDistanceFromGround(meTransform) > 0.3f || this.navAgent.isOnNavMesh == false) {
        ...
        if (this.meRigidbody == null && this.meTransform.gameObject.GetComponent<ABC_Gravity>() == null)
            this.meTransform.gameObject.AddComponent<ABC_Gravity>();
